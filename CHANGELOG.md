...
- `Bootstrap\Runner::initEnvironment()` now also sets `display_errors`/`display_startup_errors`, a `en_US.utf-8` locale, `mbstring` internal encoding/substitute character defaults, clears the stat cache, and (when the `xdebug` extension is loaded) tunes `xdebug.cli_color`/`xdebug.dump_globals`/`xdebug.show_local_vars`/`xdebug.max_nesting_level`/`xdebug.var_display_max_depth` for readable CLI debugging. `error_reporting(E_ALL)` and the `UTC` timezone it already set are unchanged.
